// Step 1: Import carLib
const Car = require("./carLib");

// Step 2: Get All cars (GET /cars)
const getAllCars = (req, res) => {
  const cars = Car.getAll();
  res.json(cars);
};

// Step 3: Create a new car (POST / cars)
const createCar = (req, res) => {
  const { model, color, age } = req.body;

  const newCar = Car.addOne(model, color, age);

  if (newCar) {
    res.json(newCar);
  } else {
    res.status(500).json({ message: "Failed to create car" });
  }
};

// Step 4: Get a car by ID (GET /cars/:carId)
const getCarById = (req, res) => {
    const carId = req.params.carId;
    const car = Car.findById(carId);
    if (car) {
      res.json(car);
    } else {
      res.status(404).json({ message: 'Car not found' });
    }
  };

// Step 5: Update a Car by ID PUT /cars/:carId)
const updateCar = (req, res) => {
    const carId = req.params.carId;
  
    const { model, color, age } = req.body;
  
    const updatedCar = Car.updateOneById(carId, { model, color, age });
  
    if (updatedCar) {
      res.json(updatedCar);
    } else {
      res.status(404).json({ message: "Car not found" });
    }
  };

// Step 6: Delete a Car by ID (DELETE /cars/:carId)
const deleteCar = (req, res) => {
    const carId = req.params.carId;
  
    const isDeleted = Car.deleteOneById(carId);
  
    if (isDeleted) {
      res.json({ message: "Car deleted successfully" });
    } else {
      res.status(404).json({ message: "Car not found" });
    }
  };

// Export the Handlers
module.exports = {
    getAllCars,
    getCarById,
    createCar,
    updateCar,
    deleteCar,
};
